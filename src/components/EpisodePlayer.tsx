...
function TeamHistoryTab({ schemaName }: { schemaName: string }) {
...
function AnthemTab({ plan }: { plan: MusicPlan }) {
  return (
      ...
      <div>
        ...
        <div className="flex flex-wrap gap-1.5">
          {plan.positiveGlobalStyles.map((s) => (
            <span key={s} className="text-[10px] px-2 py-0.5 rounded-full bg-[var(--accent)]/10 text-[var(--accent)] border border-[var(--accent)]/20">{s}</span>
          ))}
          {plan.negativeGlobalStyles.map((s) => (
            <span key={s} className="text-[10px] px-2 py-0.5 rounded-full bg-[var(--border)] text-[var(--text-muted)] line-through">{s}</span>
          ))}
        </div>
      </div>
        ...
        {plan.sections.map((section) => (
          <div key={section.sectionName} className="bg-[var(--bg)] rounded-lg p-3 border border-[var(--border)]">
            <div className="flex items-center justify-between mb-2">
              ...
              <span className="text-[10px] text-[var(--text-muted)]">{(section.durationMs / 1000).toFixed(0)}s</span>
            </div>
  ...
  return (
    <div className="w-full max-w-2xl flex flex-col gap-4 animate-slide-up">
      ...
      <div className="bg-[var(--surface)] border border-[var(--border)] rounded-xl overflow-hidden">
        ...
        {/* Anthem tab */}
        {activeTab === "anthem" && currentEpisode.musicPlan && (
          <AnthemTab plan={currentEpisode.musicPlan} />
        )}

        {/* Team History tab */}
        {activeTab === "team" && (
          <TeamHistoryTab schemaName={currentEpisode.schemaName} />
        )}
