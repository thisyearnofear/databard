  ...
  return (
    <section className="flex flex-col gap-4" aria-label="Source health evidence">
      {cards.map((card) => {
        ...
        const finding = findingSentence(card.insight);

        return (
          <article
            key={card.name}
            id={`source-${card.name.toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/(^-|-$)/g, "") || "source"}`}
            onMouseEnter={() => onHoverChange(card.name)}
            onMouseLeave={() => onHoverChange(null)}
            className="hover-depth scroll-mt-8 bg-[var(--surface)] border border-[var(--border)] rounded-2xl p-5 hover:border-[var(--accent)]/50 transition-colors"
          >
            <div className="flex items-start justify-between gap-4 flex-wrap">
              ...
              <div className="flex gap-2 items-center">
                {episodeId && <DitherButton color="purple" variant="solid" onClick={() => onListen(episodeId)} className="px-4 py-2 text-sm font-medium"><PixelIcon name="play" size={10} />Listen</DitherButton>}
              </div>
            </div>

            {card.insight && <details className="group border-t border-[var(--border)] mt-4 pt-3" onToggle={(e) => { if ((e.target as HTMLDetailsElement).open) track("story_expand", { surface: "protocol", from: "source-card" }); }}>
              <summary className="text-xs font-medium text-[var(--text-muted)] cursor-pointer list-none flex items-center gap-1.5 hover:text-[var(--text)]">
                ...
                Details · {new Date(card.insight.recordedAt).toLocaleDateString()}
              </summary>
              <div className="mt-3 space-y-3">
                <div className="flex items-center gap-6 flex-wrap">
                  ...
                  <span className="text-xs text-[var(--text-muted)] font-mono">{card.insight.tableCount} table{card.insight.tableCount !== 1 ? "s" : ""}</span>
                  ...
                  {episodeId && <Link href={`/episode/${episodeId}`} className="text-[var(--text-muted)] hover:text-[var(--accent)] text-xs transition-colors">Full report →</Link>}
                </div>
