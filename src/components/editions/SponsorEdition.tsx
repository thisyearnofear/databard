...
function formatBaseUnits(amountBaseUnits: string, decimals: number): string {
  const value = Number(amountBaseUnits) / 10 ** decimals;
  return value.toLocaleString("en-US", { maximumFractionDigits: Math.min(decimals, 9) });
}
...
export function SponsorEdition({ edition, published, showAcquisition = true }: SponsorEditionProps) {
  const { focus } = edition;
    ...
    <>
      ...
      {published && (
        <div className="mt-5 border border-[var(--palm)]/40 bg-[var(--palm)]/10 px-4 py-3 flex items-center gap-3 flex-wrap">
          ...
          <p className="text-xs text-[var(--text-muted)]">
            Commissioned edition · {published.payment
              ? `paid ${formatBaseUnits(published.payment.amountBaseUnits, published.payment.decimals)} ${published.payment.method.toUpperCase()}`
              : `Payment confirmed · $${published.pricePusd} report`}{" "}
            by{" "}
            <span className="font-mono">
              {published.paidBy.slice(0, 4)}…{published.paidBy.slice(-4)}
            </span>{" "}
            · published {new Date(published.publishedAt).toLocaleDateString("en-GB", { day: "numeric", month: "short", year: "numeric", timeZone: "UTC" })}
          </p>
          <a
      ...
      >
        <div className="flex items-start justify-between gap-4">
          <div className="min-w-0">
            ...
            <h2 id="focus-headline" className="text-xl md:text-2xl font-bold mt-3 leading-snug">
      ...
      <section className="mt-8 grid gap-6 sm:grid-cols-2">
        <div className="border border-[var(--border)] bg-[var(--surface)] px-5 py-5">
          <h2 className="text-sm font-semibold">
            {published ? `Open at publication — ${focus.name}` : `Live from ${focus.name} right now`}
          </h2>
          {focus.liveNow.length === 0 ? (
            <div className="mt-3">
              <p className="text-xs leading-relaxed text-[var(--text-muted)]">
                {published
                  ? "No listings were open in this published snapshot."
                  : `Nothing open at this moment. ${focus.short} publishes in bursts — the streak and the totals above are the better signal.`}
              </p>
        ...
        <div className="mt-8 border border-[var(--border)] bg-[var(--surface)] px-5 py-4">
          <p className="text-xs leading-relaxed text-[var(--text-muted)]">
            <span className="font-semibold text-[var(--text)]">
              {published
                ? `${edition.totals.liveNow} listings open at publication`
      ...
      {showAcquisition && (
        ...
        >
          ...
          <Link
            href="/earn"
            className="mt-4 inline-flex items-center gap-2 rounded-lg bg-[var(--accent)] px-5 py-3 text-sm font-semibold text-[var(--bg)] transition hover:brightness-110"
          >
      ...
      <section className="mt-10 border-t-2 border-[var(--border)] pt-6 text-center">
        ...
        <p className="mx-auto mt-4 max-w-[64ch] text-xs leading-relaxed text-[var(--text-muted)]">
      ...
      <p className="mt-8 text-xs leading-relaxed text-[var(--text-muted)]">
        {published
          ? `Published snapshot, observed ${new Date(edition.observedAt).toLocaleDateString("en-GB", { day: "numeric", month: "short", year: "numeric", timeZone: "UTC" })}. `
...
export function SponsorEditionShareRow({
  edition,
  slug,
  published = false,
}: {
  ...
  return (
    <ShareRow
      ...
      source="edition"
      slug={slug ?? edition.permalink.split("/").filter(Boolean).pop()}
      published={published}
    />
  );
}
